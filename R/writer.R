...
wrSVfix <- function() {
  glue::glue('### Useful stuff \n',
             ...
             '  if (is.null(props$gene_lookup_filename)) return(NULL) \n',
             '  ll <- readLines(props$gene_lookup_filename) \n',
             '  gene_lookup <- list() \n',
             '  for (l in ll) {{ \n',
             '    gg <- unlist(strsplit(l, split = "\\t")) \n',
             '    gg_to <- URLdecode(gg[1]) \n',
             '    gg_from <- URLdecode(gg[2]) \n',
             '    gg_from <- unlist(strsplit(gg_from, ",")) \n',
             '    for (ggf in gg_from) {{ \n',
             '      if (ggf %in% names(gene_lookup)) {{ \n',
             '        gene_lookup[[ggf]] <- paste0(gene_lookup[[ggf]], ",", gg_to) \n',
             '      }} else {{ \n',
             '        gene_lookup[[ggf]] <- gg_to \n',
             '      }} \n',
             '      # also allow reverse lookup: use the first match \n',
             '      if (!(gg_to %in% names(gene_lookup))) gene_lookup[[gg_to]] <- ggf \n',
             '    }} \n',
             '  }} \n',
             '  gene_lookup \n',
             '}} \n',
             'gene_lookup_table <- readGeneLookupTable() \n',
             ' \n',
             '# Get linkouts from gene_linkouts microservice (requires full-yuck gene names) \n',
             'toLinkouts <- function(gene) {{ \n',
             '  if (!is.null(gene_lookup_table) && gene %in% names(gene_lookup_table)) {{ \n',
             '    genes <- gene_lookup_table[[gene]] \n',
             '  }} else {{ \n',
             '    genes = gene \n',
             '  }} \n',
             '  if (!is.null(props$gene_prefix)) {{ \n',
             '    # prepend gene prefix for gene_linkouts call \n',
             '    genes <- unlist(strsplit(genes, split = ",")) \n',
             '    genes <- paste0(props$gene_prefix, genes) \n',
             '    genes <- paste(genes, collapse = ",") \n',
             '  }} \n',
             '  linkouts_url <- paste0("https://services.lis.ncgr.org/gene_linkouts?genes=", genes) \n',
             '  btg <- basicTextGatherer() \n',
             '  curlPerform(url = linkouts_url, writefunction = btg$update) \n',
             '  linkouts <- fromJSON(btg$value()) \n',
             '  if (length(linkouts) > 0) {{ \n',
             '    if (is.null(props$gene_prefix)) {{ \n',
             '      linkouts_text <- linkouts$text \n',
             '    }} else {{ \n',
             '      # remove gene prefix for display \n',
             '      linkouts_text <- gsub(props$gene_prefix, "", linkouts$text) \n',
             '    }} \n',
             '    linkouts_html <- paste( \n',
             '      "<dl>", \n',
             '      paste(sprintf("<dt><a href=\'%s\' target=\'_blank\'>%s</a>", linkouts$href, linkouts_text), collapse = "\\n"), \n',
             '      "</dl>", \n',
             '      sep = "\\n" \n',
             '    ) \n',
             '  }} else {{ \n',
             '    linkouts_html <- sprintf("<p>No linkouts found for %s</p>\\n", gene) \n',
             '  }} \n',
             '  linkouts_html \n',
             '}} \n',
             ' \n',
             ...
             '  \n',
             '  # Actual data.table \n',
             '  by_columns <- c("group", "sub") \n',
             '  ggData[, nCells := .N, by = by_columns] \n',
             '  ggData[, express := (val2 > 0)] \n',
             '  ggData[express == FALSE, nExpress := 0] \n',
             '  ggData[express == TRUE, nExpress := .N, by = by_columns] \n',
             '  ggData[, avgExpress := mean(val2), by = by_columns] \n',
             '  ggData[, pctExpress := 100*nExpress/nCells] \n',
             '  keep_columns <- c("group", "sub", "nCells", "nExpress", "avgExpress", "pctExpress") \n',
             '  ggData <- unique(ggData[express == TRUE, ..keep_columns])[order(group, sub)] \n',
             '  return(ggData) \n',
             '}} \n',
...
wrSVmain <- function(prefix, subst = "") {
  glue::glue('optCrt="{{ option_create: function(data,escape) {{return(\'<div class=\\"create\\"><strong>\' + \'</strong></div>\');}} }}" \n', 
             ...
             '  if (!(gene2 %in% names({prefix}gene))) gene2 <- gene_lookup_table[[gene2]] \n',
             '  updateSelectizeInput(session, "{prefix}a1inp2", choices = names({prefix}gene), server = TRUE, \n',
             '                       selected = gene1, options = list( \n',
             ...
             '      formatRound(columns = c("avgExpress", "pctExpress"), digits = c(3, 2)) \n',
             '  }}) \n',
             '  output${prefix}a1lnk2 <- renderUI(HTML(toLinkouts(input${prefix}a1inp2))) \n',
             '   \n',
             ...
             '  }}) \n',
             '  output${prefix}a3lnk1 <- renderUI(HTML(toLinkouts(input${prefix}a3inp1))) \n',
             ...
             '             input${prefix}a3fsz, input${prefix}a3asp, input${prefix}a3txt) \n',
             '  }}) \n',
             '  output${prefix}a3lnk2 <- renderUI(HTML(toLinkouts(input${prefix}a3inp2))) \n',
             ...
             '  }}) \n',
             '  output${prefix}b2lnk1 <- renderUI(HTML(toLinkouts(input${prefix}b2inp1))) \n',
             ...
             '  }}) \n',
             '  output${prefix}b2lnk2 <- renderUI(HTML(toLinkouts(input${prefix}b2inp2))) \n',
             ...
             '    datatable(ggData, rownames = FALSE, extensions = "Buttons", \n',
             '              options = list(pageLength = -1, dom = "tB", buttons = c("copy", "csv", "excel")), escape = FALSE) %>% \n',
...
wrUImain <- function(prefix, subst = "", ptsiz = "1.25") {
  glue::glue('  ### Tab1.a1: cellInfo vs geneExpr on dimRed \n',
             ...
             '        fluidRow(column(12, htmlOutput("{prefix}a1lnk2"))) \n',
             '      )  # End of column (6 space) \n',
             '    )    # End of fluidRow (4 space) \n',
             '  ),     # End of tab (2 space) \n',
             ' \n',
             ...
             '                         min = 4, max = 20, value = 8, step = 0.5)), \n',
             '        fluidRow(column(12, htmlOutput("{prefix}a3lnk1"))) \n',
             '      ), # End of column (6 space) \n',
             '      column( \n',
             ...
             '                         min = 4, max = 20, value = 8, step = 0.5)), \n',
             '        fluidRow(column(12, htmlOutput("{prefix}a3lnk2"))) \n',
             '      )  # End of column (6 space) \n',
             '    )    # End of fluidRow (4 space) \n',
             '  ),     # End of tab (2 space) \n',
             ' \n',
