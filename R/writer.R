...
wrSVfix <- function() {
  glue::glue('### Useful stuff \n',
             ...
             'gene_prefix <- readLines("gene_prefix.txt") \n',
             'toLinkouts <- function(gene) {{ \n',
             '  linkouts_url <- paste0("https://services.lis.ncgr.org/gene_linkouts?genes=", gene_prefix, gene) \n',
             '  btg <- basicTextGatherer() \n',
             '  curlPerform(url = linkouts_url, writefunction = btg$update) \n',
             '  linkouts_json <- btg$value() \n',
             '  linkouts <- fromJSON(linkouts_json) \n',
             '  linkouts_html <- sprintf("<p>No linkouts found for %s</p>\n", gene) \n',
             '  if (length(linkouts) > 0) {{ \n',
             '    linkouts_html <- paste( \n',
             '      "<dl>", \n',
             '      paste(sprintf("<dt><a href=\'%s\' target=\'_blank\'>%s</a>", linkouts$href, gsub(gene_prefix, "", linkouts$text)), collapse = "\n"), \n',
             '      "</dl>", \n',
             '      sep = "\n" \n',
             '    ) \n',
             '  }} \n',
